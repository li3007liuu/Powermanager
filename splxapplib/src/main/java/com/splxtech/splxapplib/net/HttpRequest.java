...
import com.splxtech.splxapplib.net.phapi.PhalApiClientResponse;
import com.splxtech.splxapplib.utils.BaseUtils;
import com.splxtech.splxapplib.utils.FrameConstants;


public class HttpRequest implements Runnable {
	private final static String cookiePath = "/data/data/com.splxtech.powermanagor/cache/cookie";
	// 区分get还是post的枚举
		...
		try {
			...
			if ((requestCallback != null)) {
				if (statusCode == HttpStatus.SC_OK) {
					...
					//将请求的格式替换成PhalApiResponse
					final PhalApiClientResponse phalApiClientResponse = JSON.parseObject(strResponse,PhalApiClientResponse.class);
					if(phalApiClientResponse.getRet()!=200)
					{
						//在此添加cooker失效的处理
						handleNetworkError(phalApiClientResponse.getMsg());
					}
					else
					//把成功获取的数据记录到缓存
					{
						...
						handler.post(new Runnable() {
							...
							@Override
							public void run() {
								requestCallback.onSuccess(phalApiClientResponse.getData());
							}
