package com.splxtech.powermanagor.ui;
...
public class CircularRingPercentageView extends View {
    ...
    private float lineWidth = 2f;//修改虚线条间隔
    ...
    public CircularRingPercentageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        roundBackgroundColor = mTypedArray.getColor(R.styleable.CircularRing_roundColor, 0xffffffff);//0xffdddddd
        otherRoundColor = mTypedArray.getColor(R.styleable.CircularRing_roundColor, 0xffdddddd);
        ...
        colors[0] = mTypedArray.getColor(R.styleable.CircularRing_circleColor1, 0xff5AA9F9);//0xffff4639
        colors[1] = mTypedArray.getColor(R.styleable.CircularRing_circleColor2, 0xff5AA9F9);//0xffcdd513
        colors[2] = mTypedArray.getColor(R.styleable.CircularRing_circleColor3, 0xff5AA9F9);//0xff3cdf5f
        initView();
        mTypedArray.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isLine) {
            ...
            p = (int) (progress * p);
            paint.setColor(otherRoundColor);
            canvas.drawArc(oval, -90, (float) (-(100 - progress) * 3.6), false, paint);
            for (int i = 0; i < maxColorNumber; i++) {//i < progress
                paint.setColor(roundBackgroundColor);
