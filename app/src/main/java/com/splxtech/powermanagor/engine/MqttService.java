...
public class MqttService extends Service implements MqttCallback
{
    ...
    private static final String MQTT_BROKER = "123.56.29.115"; // Broker URL or IP Address
    ...
    private synchronized void connect() {
        ...
        final PMUserInfo pmUserInfo = app.getPmUserInfo();
        try {
            if(mDataStore != null) {
        ...
        mConnHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mClient.subscribe(YunService.frtopic+ Utils.getAndroidMac(MqttService.this),0);
    ...
    public void messageArrived(MqttTopic topic, MqttMessage message)
        ...
        intent.putExtra("Message",message.getPayload());//new String(message.getPayload()));
    ...
    private IMqttService.Stub iMqttService = new IMqttService.Stub(){
        ...
        public boolean mqttPubMessage(String topic,String Message,int mqttQOS)
        {
            ...
            if(isConnected())
            {
                ...
                try {
                    cc = Message.getBytes("ISO-8859-1");
                    /*String mm="";
                    for(int i=0;i<cc.length;i++)
                    {
                        mm = mm+cc[i]+"\t";
                    }
                    Log.i("SendMessage",mm);*/
                    MqttMessage message = new MqttMessage(cc);
